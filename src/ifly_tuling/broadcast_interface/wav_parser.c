...
void WAV_P_PrintHeader(WAVContainer_t *container) {
	...
	printf("File Magic:         [%c%c%c%c]/n", (char) (container->header.magic),
	...
	printf("File Type:          [%c%c%c%c]/n", (char) (container->header.type),
			(char) (container->header.type >> 8),
			(char) (container->header.type >> 16),
			(char) (container->header.type >> 24));
	printf("/n");
	printf("Fmt Magic:          [%c%c%c%c]/n", (char) (container->format.magic),
	...
	printf("Chunk Type:         [%c%c%c%c]/n", (char) (container->chunk.type),
...
int WAV_WriteHeader(int fd, WAVContainer_t *container) {
	...
	if (write(fd, &container->header, sizeof(container->header))
			!= sizeof(container->header)
			|| write(fd, &container->format, sizeof(container->format))
					!= sizeof(container->format)
			|| write(fd, &container->chunk, sizeof(container->chunk))
					!= sizeof(container->chunk)) {
